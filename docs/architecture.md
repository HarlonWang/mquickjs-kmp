...
- 上下文单线程。`JsEngine` 不做同步，调用方在单线程使用或自行串行化；`interrupt()` 是唯一可跨线程调用的成员。`JsRuntime` 把所有访问串行到 `Dispatchers.Default.limitedParallelism(1)`（引擎没有线程亲和，只要互斥即可），协程取消与 `withTimeout` 通过一个看门狗协程转成 `interrupt()`。
